...
@dataclass
class GameConfig:
    ticks: int = 180  # 15 minutes real time (180 ticks × 5s = 900s)
    display_time_multiplier: float = 6.0  # Display as 90 min (15 min × 6)

    # Poisson means per tick (TOTAL across both teams)
    # Scaled for 180 ticks to give E[total oranges] ≈ 16 and E[total apples] ≈ 13
    # resulting in E[product] ≈ 208 with good variance
    lambda_oranges: float = 0.0889
    lambda_lemons: float = 0.0722
    ...
    # Market analysis quiz pauses
    market_quiz_probability: float = 0.12  # 12% chance per tick for market quiz
    market_quiz_enabled: bool = True
    ...
    def current_quotes(self) -> Tuple[int, Dict[str, int]]:
        ...
        for sid, sb in self.sidebets.items():
            ...
            if sb.kind == 'pow2_oranges_A':
                ...
                ev = min(ev_oranges_squared(self.tally, self.remaining_mu_oranges), 999999)
            elif sb.kind == 'apples_squared':
                ev = min(ev_apples_squared(self.tally, self.remaining_mu_lemons), 999999)
            elif sb.kind == 'oranges_A_squared':
                ev = min(ev_oranges_A_squared(self.tally, mu_A_oranges), 999999)
    ...
    def maybe_spawn_sidebet(self):
        ...
        choices: List[Tuple[str, str, SideBetFn]] = [
            ...
            ('oranges_squared', '(Total 🍊)²',
             lambda t: min(t.oranges_total ** 2, 999999)),
            ('apples_squared', '(Total 🍎)²',
             lambda t: min(t.lemons_total ** 2, 999999)),
            ('oranges_A_squared', '(Team A 🍊)²',
             lambda t: min(t.oranges_A ** 2, 999999)),
...
class TradingGUI:
    ...
    def game_timer_thread(self):
        while self.game_running:
            ...
            with self.update_lock:
                ...
                # Calculate quotes AFTER spawning to ensure new side bets have prices
                if self.engine.sidebets:
    ...
    def update_countdown(self):
        ...
        try:
            if self.engine.game_start_time:
                ...
                # Display as game time (6x faster: 15 min real = 90 min game)
    ...
    def add_sidebet_widget(self, sid: str):
        # Get side bet info with lock AND ensure price is calculated
        with self.update_lock:
            sb = self.engine.sidebets.get(sid)
            if not sb:
                return
            # If price not set yet, calculate quotes first
            if sb.price == 0:
                _, _ = self.engine.current_quotes()
                sb = self.engine.sidebets.get(sid)  # Get again after quote update
                if not sb:
                    return
            # Copy needed data
    ...
    def show_market_quiz(self):
        """Pause game and quiz user on a specific market opportunity."""
        # Select a market to quiz on (main or random side bet)
        with self.update_lock:
            available_markets = ['main']
            if self.engine.sidebets:
                available_markets.extend(list(self.engine.sidebets.keys()))
            
            import random
            selected_market = random.choice(available_markets)
            
            if selected_market == 'main':
                market_type = 'main'
                current_price = self.last_main_px
                description = "Main Market: Oranges × Apples"
                
                # Calculate probabilities for main market
                t = self.engine.tally
                remaining_ticks = self.cfg.ticks - self.engine.tick
                
                # Expected values
                exp_oranges = t.oranges_total + remaining_ticks * self.cfg.lambda_oranges
                exp_apples = t.lemons_total + remaining_ticks * self.cfg.lambda_lemons
                ev = exp_oranges * exp_apples
                
                # Estimate min/max for main
                min_oranges = max(0, int(exp_oranges - 2 * math.sqrt(remaining_ticks * self.cfg.lambda_oranges)))
                max_oranges = int(exp_oranges + 2 * math.sqrt(remaining_ticks * self.cfg.lambda_oranges))
                min_apples = max(0, int(exp_apples - 2 * math.sqrt(remaining_ticks * self.cfg.lambda_lemons)))
                max_apples = int(exp_apples + 2 * math.sqrt(remaining_ticks * self.cfg.lambda_lemons))
                
                min_payoff = min_oranges * min_apples
                max_payoff = max_oranges * max_apples
                
                # For a BUY (long position)
                buy_max_pnl = int(max_payoff - current_price)
                buy_min_pnl = int(min_payoff - current_price)
                
                # Probability of profit (simplified)
                prob_above_price = 0.5 if current_price <= ev else 0.3
                
                quiz_data = {
                    'description': description,
                    'current_price': current_price,
                    'ev': int(ev),
                    'buy_max_pnl': buy_max_pnl,
                    'buy_min_pnl': buy_min_pnl,
                    'prob_profit_buy': prob_above_price,
                    'market_type': 'main'
                }
            else:
                # Side bet
                sb = self.engine.sidebets[selected_market]
                market_type = 'sidebet'
                current_price = self.last_side_px.get(selected_market, 0)
                description = f"{selected_market}: {sb.description}"
                
                # Calculate payoff at current tally
                t = self.engine.tally
                current_payoff = sb.payoff(t)
                
                # Estimate min/max payoffs (simplified)
                min_payoff = int(current_payoff * 0.7)
                max_payoff = int(current_payoff * 1.3)
                
                # For side bets
                buy_max_pnl = max_payoff - current_price
                buy_min_pnl = min_payoff - current_price
                
                # EV
                ev = current_payoff
                
                # Probability (simplified)
                if current_price < current_payoff:
                    prob_profit_buy = 0.6
                elif current_price > current_payoff:
                    prob_profit_buy = 0.4
                else:
                    prob_profit_buy = 0.5
                ...
                quiz_data = {
                    ...
                    'buy_max_pnl': buy_max_pnl,
                    'buy_min_pnl': buy_min_pnl,
                    'prob_profit_buy': prob_profit_buy,
                    'market_type': 'sidebet',
                    'sidebet_id': selected_market
                }
        
        # Create quiz dialog
        dialog = tk.Toplevel(self.root)
        dialog.title("Market Analysis Quiz - Game Paused")
        dialog.geometry("900x750")
        dialog.configure(bg='#1a1a2e')
        dialog.transient(self.root)
        dialog.grab_set()
        
        tk.Label(dialog, text="🎯 MARKET ANALYSIS QUIZ 🎯", 
                font=('Arial', 20, 'bold'), bg='#1a1a2e', 
                fg='#00ff00').pack(pady=20)
        
        tk.Label(dialog, text="Game paused. Analyze this market opportunity:", 
                font=('Arial', 12), bg='#1a1a2e', fg='#ffffff').pack(pady=5)
        
        # Market info box (highlighted)
        info_frame = tk.Frame(dialog, bg='#ffd700', relief=tk.RAISED, bd=5)
        info_frame.pack(pady=20, padx=30, fill=tk.X)
        
        tk.Label(info_frame, text=quiz_data['description'], 
                font=('Arial', 16, 'bold'), bg='#ffd700', 
                fg='#000000').pack(pady=10)
        tk.Label(info_frame, text=f"Current Price: {quiz_data['current_price']:,}", 
                font=('Arial', 14), bg='#ffd700', 
                fg='#000000').pack(pady=5)
        
        # Questions frame
        q_frame = tk.Frame(dialog, bg='#16213e')
        q_frame.pack(pady=10, padx=30, fill=tk.BOTH, expand=True)
        
        user_answers = {}
        
        # Question 1: Probability of profit if BUY
        tk.Label(q_frame, text="1. If you BUY this market, what is your probability of profit? (0-100%)", 
                font=('Arial', 11, 'bold'), bg='#16213e', fg='#ffffff', 
                wraplength=800, justify=tk.LEFT).grid(row=0, column=0, sticky=tk.W, pady=10, padx=10)
        ans1 = tk.Entry(q_frame, font=('Arial', 12), width=20)
        ans1.grid(row=0, column=1, pady=10, padx=10)
        user_answers['prob_buy'] = ans1
        
        # Question 2: Maximum PnL if BUY
        tk.Label(q_frame, text="2. If you BUY, what is your MAXIMUM potential PnL?", 
                font=('Arial', 11, 'bold'), bg='#16213e', fg='#ffffff', 
                wraplength=800, justify=tk.LEFT).grid(row=1, column=0, sticky=tk.W, pady=10, padx=10)
        ans2 = tk.Entry(q_frame, font=('Arial', 12), width=20)
        ans2.grid(row=1, column=1, pady=10, padx=10)
        user_answers['max_pnl_buy'] = ans2
        
        # Question 3: Minimum PnL if BUY
        tk.Label(q_frame, text="3. If you BUY, what is your MINIMUM potential PnL?", 
                font=('Arial', 11, 'bold'), bg='#16213e', fg='#ffffff', 
                wraplength=800, justify=tk.LEFT).grid(row=2, column=0, sticky=tk.W, pady=10, padx=10)
        ans3 = tk.Entry(q_frame, font=('Arial', 12), width=20)
        ans3.grid(row=2, column=1, pady=10, padx=10)
        user_answers['min_pnl_buy'] = ans3
        
        # Question 4: Expected Value
        tk.Label(q_frame, text="4. What is the Expected Value (EV) of this market?", 
                font=('Arial', 11, 'bold'), bg='#16213e', fg='#ffffff', 
                wraplength=800, justify=tk.LEFT).grid(row=3, column=0, sticky=tk.W, pady=10, padx=10)
        ans4 = tk.Entry(q_frame, font=('Arial', 12), width=20)
        ans4.grid(row=3, column=1, pady=10, padx=10)
        user_answers['ev'] = ans4
        
        # Feedback label
        feedback_label = tk.Label(dialog, text="", font=('Arial', 11), 
                                 bg='#1a1a2e', fg='#ffffff', wraplength=800)
        ...
        def check_answers():
            ...
            # Check probability
            try:
                user_prob = float(user_answers['prob_buy'].get())
                correct_prob = quiz_data['prob_profit_buy'] * 100
                diff_prob = abs(user_prob - correct_prob)
                if diff_prob <= 15:
                    feedback_text += f"✅ Probability: {user_prob:.0f}% (Correct: ~{correct_prob:.0f}%)\\n"
                else:
                    feedback_text += f"❌ Probability: {user_prob:.0f}% (Correct: ~{correct_prob:.0f}%)\\n"
            except:
                feedback_text += "❌ Probability: Invalid input\\n"
            
            # Check max PnL
            try:
                user_max = int(user_answers['max_pnl_buy'].get())
                correct_max = quiz_data['buy_max_pnl']
                if abs(user_max - correct_max) <= abs(correct_max * 0.25):
                    feedback_text += f"✅ Max PnL: {user_max:,} (Correct: ~{correct_max:,})\\n"
                else:
                    feedback_text += f"❌ Max PnL: {user_max:,} (Correct: ~{correct_max:,})\\n"
            except:
                feedback_text += "❌ Max PnL: Invalid input\\n"
            
            # Check min PnL
            try:
                user_min = int(user_answers['min_pnl_buy'].get())
                correct_min = quiz_data['buy_min_pnl']
                if abs(user_min - correct_min) <= abs(correct_min * 0.25):
                    feedback_text += f"✅ Min PnL: {user_min:,} (Correct: ~{correct_min:,})\\n"
                else:
                    feedback_text += f"❌ Min PnL: {user_min:,} (Correct: ~{correct_min:,})\\n"
            except:
                feedback_text += "❌ Min PnL: Invalid input\\n"
            
            # Check EV
            try:
                user_ev = int(user_answers['ev'].get())
                correct_ev = quiz_data['ev']
                if abs(user_ev - correct_ev) <= abs(correct_ev * 0.20):
                    feedback_text += f"✅ EV: {user_ev:,} (Correct: ~{correct_ev:,})\\n"
                else:
                    feedback_text += f"❌ EV: {user_ev:,} (Correct: ~{correct_ev:,})\\n"
            except:
                feedback_text += "❌ EV: Invalid input\\n"
            
            feedback_label.config(text=feedback_text, fg='#00ff00')
        
        # Buttons
        btn_frame = tk.Frame(dialog, bg='#1a1a2e')
        btn_frame.pack(pady=20)
        
        tk.Button(btn_frame, text="CHECK ANSWERS", font=('Arial', 12, 'bold'), 
                 bg='#4CAF50', fg='white', command=check_answers).pack(side=tk.LEFT, padx=10)
        
        def resume():
            self.game_paused = False
            dialog.destroy()
        
        tk.Button(btn_frame, text="RESUME GAME", font=('Arial', 12, 'bold'), 
                 bg='#2196F3', fg='white', command=resume).pack(side=tk.LEFT, padx=10)
